...
const ProductionPerHole = ({ filteredData, DarkMode }) => {
  ...
  const [activeMeasurements, setActiveMeasurements] = useState({
    production_per_meter: true,
    total_meterage_blasted: true
  });
  ...
  const calculateProductionPerMeter = (item) => {
    if (!item.production_ton_therotical || !item.total_drill || 
        item.production_ton_therotical === 0 || item.total_drill === 0) {
      return null;
    }
    // Formula: production_ton_therotical / total_drill
    return item.production_ton_therotical / item.total_drill;
  };

  const getTotalMeterageBlasted = (item) => {
    return item.total_drill || 0;
  };
  ...
  const processedData = useMemo(() => {
    ...
    validData = validData.filter((item) => {
      const productionPerMeter = calculateProductionPerMeter(item);
      const totalMeterageBlasted = getTotalMeterageBlasted(item);
      return productionPerMeter !== null && totalMeterageBlasted > 0;
    });
    ...
    if (timeMode === 'monthly') {
      ...
      return validData
        .map((item) => ({
          ...
          production_per_meter: calculateProductionPerMeter(item),
          total_meterage_blasted: getTotalMeterageBlasted(item),
        }))
      ...
      validData.forEach((item) => {
        ...
        if (!yearlyData[year]) {
          yearlyData[year] = {
            year,
            costs: [],
            ProductionPerMeterValues: [],
            TotalMeterageBlasted: 0,
            count: 0,
          };
        }
        yearlyData[year].costs.push(item.total_exp_cost);
        const productionPerMeter = calculateProductionPerMeter(item);
        if (productionPerMeter !== null) {
          yearlyData[year].ProductionPerMeterValues.push(productionPerMeter);
        }
        yearlyData[year].TotalMeterageBlasted += getTotalMeterageBlasted(item);
      ...
      return Object.values(yearlyData)
        .map((yearData) => ({
          ...
          production_per_meter:
            yearData.ProductionPerMeterValues.length > 0
              ? yearData.ProductionPerMeterValues.reduce((sum, sc) => sum + sc, 0) /
                yearData.ProductionPerMeterValues.length
              : null,
          total_meterage_blasted: yearData.TotalMeterageBlasted,
  ...
  const summaryStats = useMemo(() => {
    ...
    const validProductionPerMeter = processedData.filter((item) => item.production_per_meter > 0 && item.production_per_meter !== null);
    const validTotalMeterageBlasted = processedData.filter((item) => item.total_meterage_blasted > 0);
    return {
      totalCost: validCosts.reduce((sum, item) => sum + item.cost, 0),
      avgProductionPerMeter:
        validProductionPerMeter.length > 0
          ? validProductionPerMeter.reduce((sum, item) => sum + item.production_per_meter, 0) /
            validProductionPerMeter.length : 0,
      totalBlasts: processedData.length,
      totalMeterageBlasted: validTotalMeterageBlasted.reduce((sum, item) => sum + item.total_meterage_blasted, 0),
      avgTotalMeterageBlasted: validTotalMeterageBlasted.length > 0 
        ? validTotalMeterageBlasted.reduce((sum, item) => sum + item.total_meterage_blasted, 0) / validTotalMeterageBlasted.length 
        : 0,
    };
  }, [processedData]);
  ...
  const chartData = useMemo(() => {
    const data = processedData.map((item) => {
      ...
      if (activeMeasurements.production_per_meter && item.production_per_meter !== null) {
        dataPoint.production_per_meter = item.production_per_meter;
      }
      if (activeMeasurements.total_meterage_blasted && item.total_meterage_blasted !== null) {
        dataPoint.total_meterage_blasted = item.total_meterage_blasted;
      }
      return dataPoint;
    });

    // Calculate trend line for production per meter
    if (data.length > 1 && activeMeasurements.production_per_meter) {
      const validData = data.filter(d => d.production_per_meter !== null && d.production_per_meter !== undefined);
      if (validData.length > 1) {
        const n = validData.length;
        const sumX = validData.reduce((sum, _, i) => sum + i, 0);
        const sumY = validData.reduce((sum, d) => sum + d.production_per_meter, 0);
        const sumXY = validData.reduce((sum, d, i) => sum + i * d.production_per_meter, 0);
        ...
        data.forEach((item, index) => {
          if (item.production_per_meter !== null && item.production_per_meter !== undefined) {
  ...
  const handleExportSimple = async (format) => {
    ...
    try {
      if (format === 'csv') {
        ...
        const csvHeaders = ['Date', 'Production per Meter (tons/m)', 'Total Meterage Blasted (m)', 'Trendline', 'Cost'];
        const csvData = chartData.map(item => [
          item.displayDate || '',
          item.production_per_meter ? item.production_per_meter.toFixed(2) : '',
          item.total_meterage_blasted ? item.total_meterage_blasted.toString() : '',
        ...
        a.href = url;
        a.download = `production-per-meter-analytics-${new Date().toISOString().split('T')[0]}.csv`;
        ...
        if (format === 'svg') {
          ...
          const a = document.createElement('a');
          a.href = url;
          a.download = `production-per-meter-analytics-${new Date().toISOString().split('T')[0]}.svg`;
          document.body.appendChild(a);
          a.click();
          document.body.removeChild(a);
          URL.revokeObjectURL(url);
          ...
          img.onload = () => {
            ...
            if (format === 'pdf') {
              ...
              pdf.save(`production-per-meter-analytics-${new Date().toISOString().split('T')[0]}.pdf`);
              
            } else {
              const link = document.createElement('a');
              link.download = `production-per-meter-analytics-${new Date().toISOString().split('T')[0]}.${format}`;
  ...
  const handleExport = async (format) => {
    ...
    try {
      if (format === 'csv') {
        // CSV Export
        const csvHeaders = ['Date', 'Specific Drilling (m³/m × 10⁴)', 'Trendline', 'Cost'];
        const csvData = chartData.map(item => [
          item.displayDate || '',
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `production-per-hole-analytics-${new Date().toISOString().split('T')[0]}.csv`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        ...
        pdf.save(`production-per-hole-analytics-${new Date().toISOString().split('T')[0]}.pdf`);
        ...
        const link = document.createElement('a');
        link.download = `production-per-hole-analytics-${new Date().toISOString().split('T')[0]}.${format}`;
        link.href = canvas.toDataURL(`image/${format}`, 1.0);
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
      ...
      return (
        <div className={`p-4 rounded-xl border ${
          ...
          <div className="space-y-1">
            {data.production_per_meter !== null && data.production_per_meter !== undefined && (
              <p className="flex items-center gap-2">
                <TrendingUp className="w-4 h-4 text-blue-500" />
                Production per Meter: {data.production_per_meter.toLocaleString('en-US', {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })} tons/m
              </p>
            )}
            {data.total_meterage_blasted !== null && data.total_meterage_blasted !== undefined && (
              <p className="flex items-center gap-2">
                <TrendingUp className="w-4 h-4 text-orange-500" />
                Total Meterage Blasted: {data.total_meterage_blasted.toLocaleString('en-US')} meters
              </p>
            )}
      ...
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <div className="flex-1">
              <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-500 to-purple-600 bg-clip-text text-transparent mb-2">
                Production per Meter Analytics Dashboard
              </h1>
        ...
        <div className={`${cardClasses} backdrop-blur-md rounded-2xl border p-6 shadow-xl`}>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <div className="space-y-2">
                <button onClick={() => toggleMeasurement('production_per_meter')}
                  className={`w-full flex items-center justify-between px-4 py-2 rounded-lg font-medium transition-all duration-300 ${activeMeasurements.production_per_meter ? 'bg-blue-500 text-white shadow-lg' : isDarkMode ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}>
                ...
                <button onClick={() => toggleMeasurement('total_meterage_blasted')}
                  className={`w-full flex items-center justify-between px-4 py-2 rounded-lg font-medium transition-all duration-300 ${activeMeasurements.total_meterage_blasted ? 'bg-orange-500 text-white shadow-lg' : isDarkMode ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}>
                  <span className="flex items-center gap-2">
                    <BarChart className="w-4 h-4" />
                    Total Meterage Blasted
                  </span>
                </button>
              </div>
            </div>
        ...
        <div className={`${cardClasses} backdrop-blur-md rounded-2xl border p-6 shadow-xl`} ref={chartRef}>
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
            ...
            Production per Meter & Total Meterage Blasted Trends
            {timeMode === 'monthly' && (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData}>
                  ...
                  <XAxis 
                    ...
                    textAnchor="end" 
                    height={60} />
                  {activeMeasurements.production_per_meter && (
                    <YAxis yAxisId="left" orientation="left" tick={{ fontSize: 12, fill: '#3B82F6' }}
                      label={{ value: 'Production per Meter (tons/m)', angle: -90, position: 'insideLeft', style: { textAnchor: 'middle', fill: '#3B82F6' } }} />
                  )}
                  {activeMeasurements.total_meterage_blasted && (
                    <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 12, fill: '#F97316' }}
                      label={{ value: 'Total Meterage Blasted (m)', angle: 90, position: 'insideRight', style: { textAnchor: 'middle', fill: '#F97316' } }} />
                  )}
                  <Tooltip content={<CustomTooltip />} />
                  <Legend />
                  {activeMeasurements.production_per_meter && (
                    <Line yAxisId="left" type="monotone" dataKey="production_per_meter" stroke="#3B82F6" name="Production per Meter" strokeWidth={2} />
                  )}
                  {activeMeasurements.production_per_meter && (
                    <Line yAxisId="left" type="monotone" dataKey="trendline" stroke="#10B981" name="Trendline" strokeWidth={1} strokeDasharray="5,5" />
                  )}
                  {activeMeasurements.total_meterage_blasted && (
                    <Line yAxisId="right" type="monotone" dataKey="total_meterage_blasted" stroke="#F97316" name="Total Meterage Blasted" strokeWidth={2} />
                  )}
                </LineChart>
              </ResponsiveContainer>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className={`${cardClasses} backdrop-blur-md rounded-2xl border p-6 shadow-xl`}>
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm opacity-75">Average Production per Meter</p>
                <p className="text-2xl font-bold">
                  {summaryStats.avgProductionPerMeter.toLocaleString('en-US', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  })}{' '}
                  tons/m
          ...
          <div className={`${cardClasses} backdrop-blur-md rounded-2xl border p-6 shadow-xl`}>
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm opacity-75">Total Meterage Blasted</p>
                <p className="text-2xl font-bold">
                  {summaryStats.totalMeterageBlasted.toLocaleString('en-US')} m
                </p>
              </div>
            </div>
          </div>
...
export default ProductionPerHole;
