...
const ProductionPerHole = ({ filteredData, DarkMode }) => {
  ...
  const [activeMeasurements, setActiveMeasurements] = useState({
    production_per_hole: true,
    total_holes_blasted: true
  });
  ...
  const calculateProductionPerHole = (item) => {
    if (!item.production_ton_therotical || !item.hole_blasted || 
        item.production_ton_therotical === 0 || item.hole_blasted === 0) {
      return null;
    }
    // Formula: production_ton_therotical / hole_blasted
    return item.production_ton_therotical / item.hole_blasted;
  };

  const getTotalHolesBlasted = (item) => {
    return item.hole_blasted || 0;
  };
  ...
  const processedData = useMemo(() => {
    ...
    validData = validData.filter((item) => {
      const productionPerHole = calculateProductionPerHole(item);
      const totalHolesBlasted = getTotalHolesBlasted(item);
      return productionPerHole !== null && totalHolesBlasted > 0;
    });
    ...
    if (timeMode === 'monthly') {
      ...
      return validData
        .map((item) => ({
          ...
          production_per_hole: calculateProductionPerHole(item),
          total_holes_blasted: getTotalHolesBlasted(item),
        }))
      ...
      validData.forEach((item) => {
        ...
        const productionPerHole = calculateProductionPerHole(item);
        if (productionPerHole !== null) {
          yearlyData[year].ProductionPerHoleValues.push(productionPerHole);
        }
        yearlyData[year].TotalHolesBlasted += getTotalHolesBlasted(item);
      ...
      return Object.values(yearlyData)
        .map((yearData) => ({
          ...
          production_per_hole:
            yearData.ProductionPerHoleValues.length > 0
              ? yearData.ProductionPerHoleValues.reduce((sum, sc) => sum + sc, 0) /
                yearData.ProductionPerHoleValues.length
              : null,
          total_holes_blasted: yearData.TotalHolesBlasted,
  ...
  const summaryStats = useMemo(() => {
    ...
    const validProductionPerHole = processedData.filter((item) => item.production_per_hole > 0 && item.production_per_hole !== null);
    const validTotalHolesBlasted = processedData.filter((item) => item.total_holes_blasted > 0);
    return {
      totalCost: validCosts.reduce((sum, item) => sum + item.cost, 0),
      avgProductionPerHole:
        validProductionPerHole.length > 0
          ? validProductionPerHole.reduce((sum, item) => sum + item.production_per_hole, 0) /
            validProductionPerHole.length : 0,
      totalBlasts: processedData.length,
      totalHolesBlasted: validTotalHolesBlasted.reduce((sum, item) => sum + item.total_holes_blasted, 0),
      avgTotalHolesBlasted: validTotalHolesBlasted.length > 0 
        ? validTotalHolesBlasted.reduce((sum, item) => sum + item.total_holes_blasted, 0) / validTotalHolesBlasted.length 
        : 0,
    };
  }, [processedData]);
  ...
  const chartData = useMemo(() => {
    const data = processedData.map((item) => {
      ...
      if (activeMeasurements.production_per_hole && item.production_per_hole !== null) {
        dataPoint.production_per_hole = item.production_per_hole;
      }
      if (activeMeasurements.total_holes_blasted && item.total_holes_blasted !== null) {
        dataPoint.total_holes_blasted = item.total_holes_blasted;
      }
      return dataPoint;
    });

    // Calculate trend line for production per hole
    if (data.length > 1 && activeMeasurements.production_per_hole) {
      const validData = data.filter(d => d.production_per_hole !== null && d.production_per_hole !== undefined);
      if (validData.length > 1) {
        const n = validData.length;
        const sumX = validData.reduce((sum, _, i) => sum + i, 0);
        const sumY = validData.reduce((sum, d) => sum + d.production_per_hole, 0);
        const sumXY = validData.reduce((sum, d, i) => sum + i * d.production_per_hole, 0);
        ...
        data.forEach((item, index) => {
          if (item.production_per_hole !== null && item.production_per_hole !== undefined) {
  ...
  const handleExportSimple = async (format) => {
    ...
    try {
      if (format === 'csv') {
        ...
        const csvHeaders = ['Date', 'Production per Hole (tons)', 'Total Holes Blasted', 'Trendline', 'Cost'];
        const csvData = chartData.map(item => [
          item.displayDate || '',
          item.production_per_hole ? item.production_per_hole.toFixed(2) : '',
          item.total_holes_blasted ? item.total_holes_blasted.toString() : '',
        ...
        if (format === 'svg') {
          ...
          const a = document.createElement('a');
          a.href = url;
          a.download = `production-per-hole-analytics-${new Date().toISOString().split('T')[0]}.svg`;
          document.body.appendChild(a);
          a.click();
          document.body.removeChild(a);
          URL.revokeObjectURL(url);
      ...
      return (
        <div className={`p-4 rounded-xl border ${
          ...
          <div className="space-y-1">
            {data.production_per_hole !== null && data.production_per_hole !== undefined && (
              <p className="flex items-center gap-2">
                <TrendingUp className="w-4 h-4 text-blue-500" />
                Production per Hole: {data.production_per_hole.toLocaleString('en-US', {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })} tons
              </p>
            )}
            {data.total_holes_blasted !== null && data.total_holes_blasted !== undefined && (
              <p className="flex items-center gap-2">
                <TrendingUp className="w-4 h-4 text-orange-500" />
                Total Holes Blasted: {data.total_holes_blasted.toLocaleString('en-US')} holes
              </p>
            )}
      ...
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <div className="flex-1">
              <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-500 to-purple-600 bg-clip-text text-transparent mb-2">
                Production per Hole Analytics Dashboard
              </h1>
        ...
        <div className={`${cardClasses} backdrop-blur-md rounded-2xl border p-6 shadow-xl`}>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <div className="space-y-2">
                <button onClick={() => toggleMeasurement('production_per_hole')}
                  className={`w-full flex items-center justify-between px-4 py-2 rounded-lg font-medium transition-all duration-300 ${activeMeasurements.production_per_hole ? 'bg-blue-500 text-white shadow-lg' : isDarkMode ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}>
                ...
                <button onClick={() => toggleMeasurement('total_holes_blasted')}
                  className={`w-full flex items-center justify-between px-4 py-2 rounded-lg font-medium transition-all duration-300 ${activeMeasurements.total_holes_blasted ? 'bg-orange-500 text-white shadow-lg' : isDarkMode ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}>
                  <span className="flex items-center gap-2">
                    <BarChart className="w-4 h-4" />
                    Total Holes Blasted
        ...
        <div className={`${cardClasses} backdrop-blur-md rounded-2xl border p-6 shadow-xl`} ref={chartRef}>
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
            ...
            Production per Hole & Total Holes Blasted Trends
            {timeMode === 'monthly' && (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData}>
                  ...
                  <XAxis 
                    ...
                    textAnchor="end" 
                    height={60} />
                  {activeMeasurements.production_per_hole && (
                    <YAxis yAxisId="left" orientation="left" tick={{ fontSize: 12, fill: '#3B82F6' }}
                      label={{ value: 'Production per Hole (tons)', angle: -90, position: 'insideLeft', style: { textAnchor: 'middle', fill: '#3B82F6' } }} />
                  )}
                  {activeMeasurements.total_holes_blasted && (
                    <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 12, fill: '#F97316' }}
                      label={{ value: 'Total Holes Blasted', angle: 90, position: 'insideRight', style: { textAnchor: 'middle', fill: '#F97316' } }} />
                  )}
                  <Tooltip content={<CustomTooltip />} />
                  <Legend />
                  {activeMeasurements.production_per_hole && (
                    <Line yAxisId="left" type="monotone" dataKey="production_per_hole" stroke="#3B82F6" name="Production per Hole" strokeWidth={2} />
                  )}
                  {activeMeasurements.production_per_hole && (
                    <Line yAxisId="left" type="monotone" dataKey="trendline" stroke="#10B981" name="Trendline" strokeWidth={1} strokeDasharray="5,5" />
                  )}
                  {activeMeasurements.total_holes_blasted && (
                    <Line yAxisId="right" type="monotone" dataKey="total_holes_blasted" stroke="#F97316" name="Total Holes Blasted" strokeWidth={2} />
                  )}
                </LineChart>
              </ResponsiveContainer>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className={`${cardClasses} backdrop-blur-md rounded-2xl border p-6 shadow-xl`}>
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm opacity-75">Average Production per Hole</p>
                <p className="text-2xl font-bold">
                  {summaryStats.avgProductionPerHole.toLocaleString('en-US', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  })}{' '}
                  tons
          ...
          <div className={`${cardClasses} backdrop-blur-md rounded-2xl border p-6 shadow-xl`}>
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm opacity-75">Total Holes Blasted</p>
                <p className="text-2xl font-bold">
                  {summaryStats.totalHolesBlasted.toLocaleString('en-US')}
                </p>
              </div>
            </div>
          </div>
